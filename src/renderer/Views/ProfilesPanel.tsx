...
function AccountProfileComponent(props: Props) {
  ...
  const [profile, setProfile] = React.useState<Profile>(initProfile);
  ...
  const handleChangeProfile = (e: React.ChangeEvent<HTMLInputElement>) => {
    e.persist();
    e.target.name != ""
      ? setProfile((profile:Profile)=>({...profile, [e.target.name]: e.target.value}))
        : setProfile((profile:Profile)=>({...profile,[e.target.id]: e.target.value}));
  };

  //handle profiles when select the profilename
  const handleChangeProfileList = (e:any) => {
  ...
  const handleCreateProfile = () => {   
    ipcRenderer.send("addprofile", profile);
  };
  ...
  const handleDeleteProfile = () => {
    ...
    ipcRenderer.send("removeprofile", profilename);
  };
  ...
  return (
    <Grid container spacing={3}>
      ...
      <div>
        ...
        <ColorButton variant="outlined" onClick={handleDeleteProfile}>
          Delete Profile
        </ColorButton>
      </div>
    </Grid>
